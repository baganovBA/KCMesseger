<script>
    ...
    const intersectionCallback = async entry=> {
        ...
        if(entry[0].isIntersecting){
            $roomsMessages[$selected_chat].scroll_to_element = entry[0].target.id
    ...
    $:{$selected_chat; last_mess_before_update_id = $selected_chat && $roomsMessages[$selected_chat].messages[0].event_id}
...
const getMoreMessages = async() =>{
    ...
    if($roomsMessages?.[$selected_chat]?.start_next_batch){
        since = $roomsMessages?.[$selected_chat]?.start_next_batch
    }

    const res_mess = await Matrix.room_messages($selected_chat, limit, since)

    loading_new_messages = false

    if(res_mess){
        $roomsMessages[$selected_chat].start_next_batch = res_mess.end
    }
    ...
    if(res_mess?.chunk?.length){
        ...
        if(res_mess?.chunk?.length < limit){
            $roomsMessages[$selected_chat].fullData = true
        }
